...
M3和M4的寄存器组一共有16个寄存器，其中`R0-R12`这13个寄存器是通用寄存器，`R13`是栈寄存器，`r14`是LR寄存器，`R15`是PC寄存器。
...
```
### 2.3.2 动态创建任务
​    静态创建任务中：控制块和任务栈的内存空间都是从内部的 SRAM 里面分配的，具体分配到哪个地址由编译器决定。而动态内存则使用**堆**，也属于 SRAM。本质为在 SRAM 里面**定义一个大数组**，也就是堆内存，供 FreeRTOS 的动态内存分配函数使用，在第一次使用的时候，系统会将定义的堆内存进行初始化。

​    栈是动态创建，**不再是全局变量**。在使用动态内存的时候，任务创建函数 xTaskCreate()会**返回一个指针指向任务控制块。**

``` c
#define configTOTAL_HEAP_SIZE ((size_t)(36*1024)) // 堆大小为36kb
static uint8_t ucHeap[ configTOTAL_HEAP_SIZE ]; //  定义堆
...
任务延时列表是有环链表，每个节点代表了正在延时的任务，**节点按照延时时间做升序排列。**
    
当每次 SysTick 中断触发，系统时基计数器的值 `xTickCount` 与解锁时刻变量 `xNextTaskUnblockTime` 的值相比较，如果相等，则表示有任务延时到期，需要将该任务就绪，否则只是单纯地更新系统时基计数器`xTickCount` 的值，然后进行任务切换。
    
延时列表初始化见**2.3.2 列表初始化与操作，每次创建任务会检测就绪列表与延时列表是否创建。**

``` c
static List_t xDelayedTaskList1;  // xTickCount未溢出
static List_t xDelayedTaskList2;  // xTickCount溢出

static List_t * volatile pxDelayedTaskList;  // 地址变量
static List_t * volatile pxOverflowDelayedTaskList;  // 地址变量
```

### 2.3.4 将任务插入就绪列表

**就绪链表的目的是找到下一个执行任务的TCB！**

**同一个优先级插入同一条链表**，默认优先级数量为5，最大支持256个优先级。
...
static void prvAddNewTaskToReadyList( TCB_t *pxNewTCB )
{
    ...
    taskENTER_CRITICAL();
    {
        ...
        /* 如果是创建的第一个任务(pxCurrentTCB为空) */
        if ( pxCurrentTCB == NULL )
        {
            pxCurrentTCB = pxNewTCB;  // 指定pxCurrentTCB(但不会运行此任务，须待调度器启用后)
            /* 如果是第一次创建任务，则需要初始化所有优先级的列表 */
            if ( uxCurrentNumberOfTasks == ( UBaseType_t ) 1 )
            	prvInitialiseTaskLists();
         }
         else  // 每次创建新任务会进行依次任务切换检测
         {
...
void vTaskStartScheduler(void){
    ...
    if(xStartScheduler() != pdFALSE){
    }
}
```

#### 2.4.2.2 设置PendSV/SysTick中断优先级
...
```
#### 2.4.2.3 开启中断/设置MSP/触发SVC中断

`prvStartFirstTask()`函数用于开始第一个任务，主要做了两个动作，一个是**更新 MSP 的值**，二是**产生 SVC 系统调用，然后去到 SVC 的中断服务函数里面真正切换到第一个任务**。

在 Cortex-M 中，`0xE000ED08` 是 `SCB_VTOR` 寄存器的地址，里面存放的是向量表在内存中的地址，即 `MSP` 的地址。`MSP`通常在内部 `FLASH` 的起始地址，那么可知 `memory：0x00000000` 处存放的就是 MSP 存储的值。
