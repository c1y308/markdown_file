...
`uxTopReadyPriority`全局变量。

---

主要干三件事：

- 如果是第一次创建任务即`pxCurrentTCB`为空则将其指定为`pxCurrentTCB`，并**初始化任务相关的所有链表**（有多少个`ReadyList`优先级链表）可以由`configMAX_PRIORITIES`这个宏得知，`xDelayedTaskList`固定为两个;
- 不是则检查其优先级是否需要将当前运行任务切换为此任务；
- 将此任务添加到`ReadyList`链表中；
...
全局变量：`xNextTaskUnblockTime`，**记录下一个任务需要解锁的时间**。
