...
- **release（释放）：** “栅栏把我之前的所有内存**写操作都关在前面**，它们谁也不能越过我跑到我后面去。”
  这样，当别人看到我这个原子写入的值时，一定能看到那些“关在后面的”写结果。
- **acquire（获取）：** “栅栏把我之后的所有内存**读操作都挡在后面**，它们谁也不能越过我跑到我前面去。”
...
`std::atomic_flag` 是无锁的原子布尔类型（**唯一强制保证无锁的原子类型**），**只提供两种操作**：
...
`static`成员变量对所有`object`都是共享的，**`static`成员变量在类中只进行声明，在类外进行定义（除非声明为`const`）**。
...
核心思想非常简单：**保证一个类仅有一个实例，并提供一个访问它的全局访问点。**

在 C++ 中实现单例，除了要考虑全局唯一性，还必须严肃对待**内存泄漏（Memory Leak）**和**多线程安全（Thread Safety）**。
...
template <bool Cond, typename T = void>
using enable_if_t = typename enable_if<Cond, T>::type;  // typename 表明是把 enable_if<Cond, T>::type 作为类型
```
...
核心特性：**读共享、写独占**。多个线程可同时持有读锁，写锁持有期间所有其他线程都被阻塞。
...
`std::once_flag` 与 `std::call_once` 是 C++11 引入的标准线程安全一次性执行机制，定义于头文件 `<mutex>`，核心作用是保证**指定函数在多线程环境下有且仅有一次成功执行**，是**单例实例创建、全局资源初始化、配置加载等场景**的标准解决方案。

#### `std::once_flag`

`std::once_flag` 是 `call_once` 的配套状态载体，本身是一个极简的标志类，不对外暴露任何操作接口，**仅用于传递给 `std::call_once` 来记录执行状态**。

- **默认构造**：创建时处于「未执行」状态。
- **封闭状态**：没有 `test/set/clear` 等公开成员函数，用户无法手动读取或修改内部状态，完全由 `std::call_once` 管理。
- **不可拷贝 / 移动**：删除了拷贝构造、移动构造与赋值运算符，只能独立创建。
- **一次性语义**：一旦关联的 `call_once` 成功执行完成，该标志永久处于「已完成」状态，无法重置复用。

``` c++
#include <mutex>
std::once_flag g_init_flag; // 通常定义为全局或静态变量
```

#### `std::call_once`

`std::call_once` 是执行一次性逻辑的核心函数，它接收一个 `once_flag` 和**一个可调用对象**，保证该可调用对象有且仅有一次成功执行。
...
- `flag`：关联的一次性状态标志，同一个业务逻辑必须共用同一个 `once_flag`。

- `f`：可调用对象（普通函数、lambda、函数对象等），其返回值会被忽略。

- `args...`：传递给 `f` 的参数，支持完美转发；传递引用参数时需用 `std::ref` 包裹，否则会发生值拷贝。

---

**异常安全保证：**这是 `call_once` 最关键也最容易被忽略的特性：

- 如果执行函数 `f` 的过程中抛出异常，异常会被传播给本次调用 `call_once` 的线程。
- 此时 `once_flag` **不会被标记为已完成**，后续的 `call_once` 调用会重新尝试执行函数，直到有一次调用正常返回。
- 这种机制保证了：初始化失败时可以安全重试，最终一定有一次成功的初始化。
