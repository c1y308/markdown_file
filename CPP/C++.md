...
类的成员函数**不修改类的非`mutable`的成员变量**立刻**在成员函数名后面加上`const`**。
...
`return by reference` 时`return`的不能是函数内部的局部变量，必须返回全局变量或者不会死亡的。
    ...
    public:
        MyString(const char *s = ""){
            ...
            if(s == nullptr){
                ...
                len = strlen(s);
                str = new char[len + 1];
                strcpy(str, s);  // strcpy函数会拷贝`/0`
            }
        }

        MyString& operator=(const MyString &other){
            std::cout << "拷贝复制函数被调用" << endl;
            if(this == &other)
                return *this;
            delete[] this->str;
        ...
        void print() const{
            std::cout << "字符串为" << (str ? str : "null") <<" "<< "地址为" << (void*) str << endl;
        }
