...
- 面对**函数**（最具有威力）在一个函数声明式内`const`可以和函数返回值、参数、函数自身（如果是成员函数）产生关联：
  ...
  - **如果是成员函数**则可以修饰**函数自身**：
    ...
    char& operator[](std::size_t position){
        return const_cast<char&>( static_cast<const TextBlock&>(*this)[position] );  // 先将*this 从TextBook& 转型为
        																			 //  const TextBook&
    }
    ```
...
- **修饰局部变量（函数或区块作用域内）**：打破生命周期限制，实现状态的“记忆”。
  ...
  void generateOrderID() {
      // 这里的 id 只有在第一次调用时初始化为 1000
      // 随后的调用会直接跳过初始化语句，保留上次递增后的值
      static int id = 1000;
      std::cout << "生成的订单号: " << id++ << std::endl;
  }
  
  int main() {
      generateOrderID(); // 输出 1000
      generateOrderID(); // 输出 1001
      return 0;
  }
  ```

- **现代 C++ 的黑魔法（Magic Statics）**：

  在 C++11 之前，多线程环境下初始化局部 `static` 变量是不安全的（可能导致竞态条件引发重复初始化）。但在 C++11 中，标准强行规定了：**局部静态变量的初始化必须是线程安全的**。编译器会在底层默默为你加锁。这使得局部 `static` 成了现代 C++ 中实现**单例模式（Singleton）**最优雅、最简洁的方法（即 Meyers' Singleton）。
...
- **C++17 的语法大救星（`inline static`）**：
  ...
  过去，静态成员变量必须在 `.h` 文件中声明，然后在某一个 `.cpp` 文件中单独定义，这种设定非常容易引发链接错误。C++17 引入了 `inline` 变量，终于允许你在类内部直接定义并初始化静态成员，清爽无比：
...
- **修饰成员函数**：剥离了 `this` 指针的类专属工具。

  普通的成员函数在调用时，编译器会暗中塞进一个 `this` 指针，让函数知道是哪个对象在调用它。而 `static` 成员函数**根本没有 `this` 指针**！

  这带来了两个极其重要的推论：

  1. 它可以不依赖任何具体的对象，直接通过作用域解析符调用（例如 `Player::printTotal()`）。
  2. **它绝对不能访问类中的非静态（non-static）成员变量或普通成员函数！**（因为没有 `this` 指针，它无从得知该去访问哪一个具体对象的数据）。
...
在理解了上述 `static` 的基础用法后，我们可以将其特性结合起来，解决 C++ 中一个臭名昭著的历史级痛点：解决**当一个类在“自身的构造（初始化）期间”，需要依赖另一个文件定义的全局变量时，如何保证那个被依赖的全局变量一定已经初始化完毕。**
...
- **痛点：无法预测的初始化顺序**
  ...
  在 C++ 中，**non-local static 对象**（不是指被`static`修饰，而是包括全局对象、命名空间作用域内的对象、以及 class 内部的 static 成员对象）如果分布在**不同的编译单元（`.cpp` 文件）**中，C++ 标准**绝没有规定**它们的初始化顺序!（函数中的`static`对象称之为`local static`对象）
...
- **破局点：用 local static 替换 non-local static**
  ...
  1. 将跨文件的全局（non-local）static 对象，搬进一个专属的全局函数内部，变成**局部（local）static 对象**。
  2. 让该函数返回这个对象的引用。
  3. 外部代码不再直接访问全局变量，而是统一调用这个函数。
